  ...
  return (
    <div className="w-full text-gray-700 bg-cream">
      <div className="flex flex-col max-w-screen-xl px-8 mx-auto md:items-center md:justify-between md:flex-row">
        ...
        <nav
          className={`${
            open ? "h-auto scale-y-100" : "h-0 scale-y-0 md:scale-y-100 md:h-auto"
        ...
        >
          {["Home", "Careers", "Blog", "About Us"].map((item) => (
            <Link
              key={item}
              className="px-4 py-2 mt-2 text-sm bg-transparent rounded-lg md:mt-8 md:ml-4 hover:text-gray-900 focus:outline-none"
              href="#"
            >
              {item}
            </Link>
